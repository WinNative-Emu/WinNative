/*
 * The two adjustments a session's opens need.
 *
 ...
 * session reads those markers, and one of the descriptors does not survive: something closes it,
 * the number is handed to the next file the client opens, and the markers land in that file
 * instead. They have been found appended to an app manifest, which the client then cannot parse,
 * to a download's state file, and to content chunks, which it quarantines as corrupt. Refused the
 * descriptor, tier0 writes nothing at all and has nothing to lose.
 */
#define _GNU_SOURCE
#include <dlfcn.h>
#include <errno.h>
